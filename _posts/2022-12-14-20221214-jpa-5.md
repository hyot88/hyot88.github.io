...
하지만 각각 호출하다보면 실수로 둘 중 하나만 호출해서 양방향이 깨질 수 있으므로 연관관계 편의 메소드로 아래처럼 코드를 리팩토링하는게 좋다.  
하나의 호출로 두 코드를 실행시켜주는 것이다.  
```java
public class Member {
    private Team team;
